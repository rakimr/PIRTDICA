...
The web platform is built with FastAPI (Python) for the backend, SQLAlchemy for ORM, and Jinja2 templates with custom CSS for the frontend, featuring live scoring, contest history, and admin controls. The single-currency system (Coach Coin) supports a Play-to-Earn (P2E) model focused on Identity, Prestige, Access, and Analytics, strictly avoiding pay-to-win mechanics. Coach Coin is earned by beating the AI-generated House lineup and through achievements. H2H matches and the shop all use Coach Coin. Ranked modes include a tiered division system, hidden MMR, and seasonal resets. Monetization relies on a small rake on H2H competitions, cosmetic sales, and Stripe subscriptions.

Three subscription tiers via Stripe: **PIRTDICA Picks** ($9/week, gates Articles), **PIRTDICA Stat Pack** ($16/week, gates Chart Gallery), and **PIRTDICA Bundle** ($60/month, both — 40% off vs buying separately at $100/month equivalent). Non-subscribers see the header image/title but content is replaced by a 3-card pricing grid with a lock icon. The Bundle card shows "$100" struck through with a green neon "40% OFF" badge. All 3 plans are prompted on signup, login, Articles paywall, and Chart Gallery paywall. Nav bar shows lock icons next to Articles and Chart Gallery links.

Multi-subscription architecture: `user_subscriptions` table (id, user_id, stripe_subscription_id, plan, status, current_period_end) supports users having multiple active subscriptions (e.g., Picks + Stat Pack separately). Legacy `users.subscription_*` fields are synced for backward compatibility. Access checks (`has_picks_access`, `has_statpack_access`, `has_any_subscription`) accept optional `db` parameter — when provided, check `UserSubscription` rows first, falling back to legacy user fields if no rows exist. Bundle upgrade automatically cancels individual Picks/Stat Pack subscriptions via Stripe API.

Stripe integration uses the Python `stripe` library with API keys from Replit's Stripe connector (loaded from `.stripe_keys.json`). Backend module: `backend/stripe_billing.py`. Routes: `/subscribe?plan=picks|statpack|bundle` (checkout), `/subscribe/success` (callback), `/stripe/webhook` (Stripe events), `/billing` (customer portal). Profile page shows current plan name(s) with Manage Billing link. Pricing cards template: `templates/pricing_cards.html` (included by articles, trends_paywall, and base.html modal).

Subscription activation triggers: in-app notification (priority 1, type "subscription", category "financial") and email via Resend queue (`subscription_activated_email` template). Emitted once per new subscription via `emit_subscription_activated` in `backend/events.py`, with idempotency via `upsert_user_subscription` returning `is_new` flag.
...
The Articles page (`/articles`) displays daily HIGH Confidence Props analysis on the web platform. It shows: the header image, a picks table (# | Player | Game | Stat | Avg | Line | Projected | Edge | Pick), and per-player analysis sections with matchup context, archetype info, DVA/DVP edges, last 5 games, projection factors, and the specific call. Content is generated by `generate_article.py` which reads `prop_recommendations.csv` and `dfs_players.csv`, builds the header image (saving to `static/images/`), and stores everything in the `daily_articles` PostgreSQL table. Integrated into the daily pipeline via `run_daily_update.py`. Re-running `generate_article.py` after injury updates regenerates the article content.
