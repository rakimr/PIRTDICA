...
Line movement tracking: Sportsbook line history lives in the SQLite staging DB (`dfs_nba.db.player_props_history`) rather than as a SQLAlchemy model in `backend/models.py`, since all line-related code paths (the scraper, `_load_line_movement` in `analysis/player_value.py`, and the article generator) read it via raw SQL. The PostgreSQL `backend/models.py` is reserved for platform tables (users, contests, subscriptions). The line-movement callout fires on both the Claude article path (`_build_pick_context` → `line_movement_signal`) and the rule-based template fallback (`build_analysis_text_template`) when `|line_drift| >= 0.5` across `>= 2` snapshots.

Intra-day props scrapes: `scheduler_props.py` runs as a persistent workflow, triggering `scrape_player_props.py --force` at 11 AM, 2 PM, and 4 PM ET. Each run appends a snapshot to `player_props_history` (no DELETE). Combined with the 1 AM post-game scrape and the pre-tip pre-game scrape, this gives 4-5 daily snapshots per prop instead of just open vs current. Rate-limit aware: the underlying scraper auto-skips when no NBA events are scheduled today. The richer history powers `_load_line_movement`'s last-hour drift, which lets the article generator surface late same-day moves (often injury news or sharp action) as a distinct narrative signal.
