...
The Articles paywall gates daily HIGH Confidence Props content behind a PIRTDICA Picks subscription ($9/week via Stripe). Non-subscribers see the header image and title but the picks table and analysis cards are replaced by a paywall card with a Subscribe button. After login/signup, non-subscribed users see a modal prompting them to subscribe. Stripe integration uses the Python `stripe` library with API keys from Replit's Stripe connector (stored as `STRIPE_SECRET_KEY` and `STRIPE_PUBLISHABLE_KEY` env vars). Backend module: `backend/stripe_billing.py`. Routes: `/subscribe` (checkout), `/subscribe/success` (callback), `/stripe/webhook` (Stripe events), `/billing` (customer portal). User model stores `stripe_customer_id`, `stripe_subscription_id`, `subscription_status`, `subscription_plan`, and `subscription_current_period_end`. Profile page shows subscription status with a "Manage Billing" link. All access checks are server-side via `is_subscriber(user)`.
