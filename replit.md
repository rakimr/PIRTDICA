...
A chart screenshot infrastructure exists via `/chart-screenshot/{chart_type}/{target}` using Playwright for batch capture. Article header images are generated via `generate_header.py` using Pillow.

The Articles page (`/articles`) displays daily HIGH Confidence Props analysis on the web platform, including a header image, picks table, and per-player analysis sections. Content is generated by `generate_article.py`.

The Stat-Specific Projection Engine generates projected values for each stat type (PTS, REB, AST, STL, BLK) using dedicated projection functions incorporating all available data sources. A Role Change Detection system (`_detect_role_change` in `analysis/player_value.py`) identifies bench-to-starter and starter-to-bench regime shifts by comparing last-5 game minutes vs season MPG (thresholds: 40%+ increase with 6+ min gap for up, 35%+ decrease with 6+ min gap for down, DNP games filtered). When detected, `_recency_weighted_avg` shifts from 60/40 (last-10/season) to 80/20 (last-5/season) weighting, and all 5 stat projection functions surface a ROLE CHANGE factor. The rebound model additionally shifts its blend to favor recent production (55/45 recent/model vs normal 45/55). Role changes are surfaced in the article narrative. The Probabilistic Minutes Model replaces the old deterministic minutes approach with an empirically-calibrated distribution-aware system. The Opportunity-Based Rebound Model uses a two-stage multiplicative architecture. The Composite Score ranks prop recommendations by combining edge size, directional DVA/DVP matchup alignment, game environment quality, consistency/CV, and trend/hit-rate alignment.

The Usage Redistribution Model v2 replaces the old flat linear injury boost with hierarchical, archetype-weighted redistribution. When teammates are OUT, it tracks each OUT player's usage, MPG, and archetype indices. Cascade tiers (0/1/2) activate based on total vacated usage (<15% / 15-35% / >35%). Archetype similarity (cosine similarity on creation, playmaking, perimeter, interior indices) determines which remaining players absorb the most usage. Minutes escalation distributes vacated minutes to remaining starters (capped at 38). The Opportunity Index = (new_minutes × new_usage) / (baseline_minutes × baseline_usage) replaces the old `avg × pct / 100` boost with a compounding multiplier. An "Opportunity Spike" flag fires when opp_index >= 1.3 AND a positive matchup signal exists, feeding into confidence evaluation. The model prevents double-counting: when Opportunity Index is active, the separate min_ratio multiplier is skipped.

The HIGH Confidence Prop Classification (tightened in Task #5) requires: hit rate >= 58%, CV <= 0.45, last-5 avg must clear the book line in the pick direction, at least one of DVA/DVP must directionally support the pick (both neutral is OK, both opposing fails), and UNDER picks with usage boost > 3.0 are blocked. Confidence reasons are always populated for both HIGH and LOW picks. Gate failure diagnostics are printed per run.
