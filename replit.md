...
Playoff awareness: `utils/season_phase.py` provides `classify_game_date()` (cutoff: month>=5 OR (month==4 AND day>=13) → 'PLAYOFF') and `is_playoff_window_active()`. `player_game_logs.season_type` is populated by both NBA.com and Basketball Reference scrapers and is indexed on (player_name, season_type, game_date DESC). When the playoff window is active, `generate_article._build_full_slate_briefing` adds per-prop `playoff_avg`, `playoff_games_count`, `playoff_min_avg`, `playoff_last_min`, `playoff_last_val`, `playoff_game_log`, `recent_includes_playoff`, and a `series_vs_opponent` block; `recent_games` matchups are tagged `[P]` / `[R]`; h2h gets `note: regular_season_only`; and a PLAYOFF MODE block is appended to the Claude system prompt instructing it to prefer playoff data over regular-season carrots. `analysis/player_value._evaluate_prop_confidence` weights playoff games at 2.5x for hit_rate/CV/avg/std, and replaces `last5_avg` with playoff-only last-5 when the player has at least 2 playoff games.
