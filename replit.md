...
- April 25, 2026: Line movement tracking shipped (Task #20). New `player_props_history` table preserves every prop-line snapshot (no DELETE on rescrape). `_load_line_movement` in `analysis/player_value.py` computes opening vs current line, drift, and snapshot count keyed by (player, stat). Composite score gets a directional ±3 modifier when drift aligns or opposes the pick. Article generator (`generate_article.py`) surfaces a `line_movement_signal` string and the Claude system prompt instructs it to interpret drift as sharp-money confirmation or yellow flag — folds in adoption candidate #4 (Claude divergence-prompt enrichment). Existing player_props rows backfilled into history one time. Drift signal becomes meaningful starting with the next pre-game refresh (second snapshot of the day).
- April 25, 2026: Mid-day props snapshots scheduler shipped (Task #23). New `scheduler_props.py` workflow fires `scrape_player_props.py --force` at 10:00, 13:00, and 15:00 ET — adds three mid-day snapshots between the existing 1 AM (post-game) and pre-tip (pre-game refresh) scrapes for 5 total snapshots per slate. `_load_line_movement` extended with `recent_drift` (current vs prior snapshot, regardless of time gap) and `recent_drift_hours` so the article can flag late sharp action separately from open-to-close drift. `_build_pick_context` adds a `late_move_signal` string when a sudden move (>=0.5) lands within the last 4 hours, and the Claude prompt instructs it to weight late moves more heavily than slow drift since they often signal breaking news. Composite score continues to use total drift only (no double-counting); `recent_drift` is narrative-only.
