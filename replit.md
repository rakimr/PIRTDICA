...
Grading is internal-only: no user-facing pick-grading displays anywhere on the site. Grades exist solely to feed Claude's pick-feedback records. Ask before re-adding any public grading/W-L display.
...
Scheduler visibility: `start_production.sh` tees each scheduler's stdout/stderr to BOTH the local file (`/tmp/pirtdica_logs/<name>.log`) AND the deployment's stdout (so `fetch_deployment_logs` shows live activity prefixed with `[scheduler_name]`). `set -m` puts each backgrounded subshell in its own process group so SIGTERM cleanup reaches supervisor+python+awk+tee on graceful shutdown. Admin-only `/admin/scheduler-status` endpoint + the "Scheduler Status (Reserved VM)" card on `/admin` show per-scheduler log freshness, state JSON, and a tail of the last 40 log lines, with a 15s auto-refresh toggle.
Launcher resilience: `start_production.sh` runs each scheduler under a SUPERVISOR loop that auto-restarts it (after `SCHEDULER_RESTART_DELAY`, default 15s) if it exits or crashes, and the script waits ONLY on the uvicorn web PID (`wait "$WEB_PID"`), not `wait -n`. The deployment's health/readiness is therefore tied to the web tier alone // a scheduler crash during the publish readiness probe can no longer tear down uvicorn and fail the health check. Previously a shared `wait -n` meant any single scheduler exit fired `cleanup` and killed the web server, which caused publishes to never become ready. If uvicorn itself exits, `cleanup` runs and the platform restarts the VM.
...
Visual aesthetic for tables/cards/charts: PIRTDICA is a LIGHT theme. When building any table, card, or chart, mirror the existing `.standings-table` pattern in `static/css/style.css` and use the `:root` CSS variables (`--bg`/`--bg-card`/`--bg-input`, `--text`/`--text-muted`, `--border`, `--primary` near-black header bars, `--wnba-accent` orange for WNBA accents). Font is `'Marvel', sans-serif`. Never hardcode dark backgrounds or light text, and never invent new colors // dark-on-light caused invisible content once. Cards: `--bg-card` bg, `1px solid var(--border)`, `border-radius: 12px`; header bar `--primary` bg + white uppercase text; status colors green `#15803d` / red `#b91c1c`.
...
The web platform is built with FastAPI (Python) for the backend, SQLAlchemy for ORM, and Jinja2 templates with custom CSS for the frontend. The single-currency system (Coach Coin) supports a Play-to-Earn (P2E) model focused on Identity, Prestige, Access, and Analytics, strictly avoiding pay-to-win. Monetization relies on rake on H2H competitions, cosmetic sales, and Stripe subscriptions. Stripe integration manages subscription tiers, webhooks, and customer portals, with Resend for notifications. New members get a 7-day card-upfront free trial on any plan (one per account, enforced by `is_trial_eligible`): Stripe auto-bills at trial end, and `trialing` status counts as entitled everywhere via `ENTITLED_STATUSES`. Per owner decision (Aug 2026), no payment-date reminders or first-charge-date disclosures are sent or shown anywhere; the ONLY trial disclosure is a "no refunds" notice at the point of starting the trial (pricing-card/billing CTA microcopy + Stripe Checkout `custom_text.submit` message) and one line in the trial-started email — deliberately WITHOUT any "cancel before the trial ends" phrasing, which the owner considers a cancellation nudge. Billing/profile show only the standard "Renews <date>" like any subscription. Manual subscription granting is available via an admin panel. Stripe SDK 15.x defaults to API version 2025-09-30+ which moved `current_period_end` from the Subscription object onto `subscription.items.data[i].current_period_end`; `backend/stripe_billing.get_subscription_period_end(sub)` reads the new location with a fallback to the legacy top-level field, and is used by every activation/renewal/recovery callsite (subscribe_success, /stripe/webhook, /billing/recover, /billing auto-recovery).
...
Static asset serving on the Reserved VM uses a custom EIO-resilient handler (`backend/static_handler.CachedStaticFiles`) instead of `fastapi.staticfiles.StaticFiles`. The default StaticFiles streams via sendfile/aiofiles and is vulnerable to transient `[Errno 5] Input/output error` on the Reserved VM filesystem (which produced unstyled "metadata-only" pages). The custom handler caches file bytes in memory keyed on (mtime_ns, size), retries OS calls on EIO with a short backoff, falls back to the stale cached copy when reads fail, and returns 503 with Retry-After only when nothing is cached. Cache-Control is `public, max-age=300`. It also tolerates Starlette Mount path-stripping differences by deduping a redundant leading `static/` segment. Memory cost is bounded by the static dir size (~18 MB).

A chart screenshot infrastructure uses Playwright. Article header images are generated via Pillow. The Articles page displays daily HIGH Confidence Props analysis. Article generation uses a three-tier system: Claude Analyst (primary), statistical model + Claude narrator (fallback), and a rule-based template engine (final fallback). Pick grading runs nightly and stores results in PostgreSQL (`wnba_daily_pick_grades` / `daily_pick_grades`) for the internal Claude pick-feedback loop ONLY — the subscriber-facing grading report was removed from /articles by owner decision (Aug 2026). All ESPN JSON fetches go through `utils/espn_fetch.espn_get_json` (default client UA + curl fallback); ESPN 403s spoofed "Mozilla/5.0" UAs since Aug 2026.
