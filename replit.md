...
Three subscription tiers via Stripe: **PIRTDICA Picks** ($9/week, gates Articles), **PIRTDICA Stat Pack** ($16/week, gates Chart Gallery), and **PIRTDICA Bundle** ($60/month, both — 40% off vs buying separately at $100/month equivalent). Non-subscribers see the header image/title but content is replaced by a 3-card pricing grid. The Bundle card shows "$100" struck through with a green neon "40% OFF" badge. All 3 plans are prompted on signup, login, Articles paywall, and Chart Gallery paywall. Stripe integration uses the Python `stripe` library with API keys from Replit's Stripe connector (loaded from `.stripe_keys.json`). Backend module: `backend/stripe_billing.py`. Routes: `/subscribe?plan=picks|statpack|bundle` (checkout), `/subscribe/success` (callback), `/stripe/webhook` (Stripe events), `/billing` (customer portal). User model stores `stripe_customer_id`, `stripe_subscription_id`, `subscription_status`, `subscription_plan` (picks/statpack/bundle), and `subscription_current_period_end`. Access checks: `has_picks_access(user)` for Articles, `has_statpack_access(user)` for Chart Gallery, `has_any_subscription(user)` for general checks. Profile page shows current plan name with Manage Billing link. Pricing cards template: `templates/pricing_cards.html` (included by articles, trends_paywall, and base.html modal).
