...
Playoff awareness: `utils/season_phase.py` provides `classify_game_date()` (cutoff: month>=5 OR (month==4 AND day>=13) → 'PLAYOFF'), `is_playoff_window_active()`, and `current_playoff_window_start()` (returns April 13 of the current calendar year when in playoff window, used to bound "current series only" lookups). `player_game_logs.season_type` is populated by both NBA.com and Basketball Reference scrapers and is indexed on (player_name, season_type, game_date DESC). `matchup_history` is partitioned by `season_type` (REGULAR + PLAYOFF rows persisted side by side); `compute_familiarity_impact` filters to REGULAR so familiarity scoring stays apples-to-apples year-round. When the playoff window is active, `generate_article._build_full_slate_briefing` adds per-prop `playoff_avg`, `playoff_games_count`, `playoff_min_avg`, `playoff_last_min`, `playoff_last_val`, `playoff_game_log`, `recent_includes_playoff`, and a `series_avg_vs_opponent` block (bounded to the current postseason); `recent_games` matchups are tagged `[PLAYOFF G{n}]` / `[REG]`; h2h gets `note: regular_season_only`; `series_role_change` carries `label` (stable/expanded/reduced via ±5 MPG threshold) and a `role_shifted` boolean; unweighted comparison fields (`hit_rate_unweighted`, `cv_unweighted`, `last5_avg_unweighted`, `playoff_n`, `playoff_weight_applied`) are surfaced for Claude. All five article-generation entry points (`_get_matchup_history`, `_build_full_slate_briefing`, `build_analysis_text_template`, `build_claude_analyst`, `build_analysis_claude`) accept a `game_date` kwarg so reruns and backfills classify against the slate date rather than wall-clock today. A PLAYOFF MODE block is appended to the Claude system prompt instructing it to prefer playoff data over regular-season carrots. `analysis/player_value._evaluate_prop_confidence` weights playoff games at 2.5x for hit_rate/CV/avg/std, and replaces `last5_avg` with playoff-only last-5 when the player has at least 2 playoff games.
